...
type requestData struct {
	...
	Values url.Values
	Files  files
	Array  map[int]map[string]string
}
...
func (d *data) setDataValue(mErr *MultiError) error {
	...
	for fieldNum := range d.typ.NumField() {
		...
		//fmt.Println(field.typ.Type.Kind() == reflect.Slice)
		if err != nil {
			return err
		}
		if field.typ.IsExported() {
			...
			//case arrayType:
			//	// что должно быть вместо arrayType?
			default:
...
func (f *field) setArrayValue(mErr *MultiError) {
	fmt.Println(f.requestData.Array)
	...
	if f.typ.Type.Elem().Kind() == reflect.Struct {
		...
		typ := val.Type()
		for k, v := range f.requestData.Array {
			_ = k
			_ = v
		}
	...
	requestArray := f.requestData.Values[f.tags.Name]
