...
type array struct {
	fieldName    string
	structArray  map[int]map[string]string
	defaultArray []string
}
...
// todo check if in map уже есть такое поле но не для массивов
...
func (d *data) fieldTags() (map[string]string, error) {
	//tags := make([]string, 0)
	...
	for f := range d.typ.NumField() {
		...
		field := field{typ: d.typ.Field(f), val: d.val.Field(f), tagType: d.tagType}
		if field.typ.IsExported() {
			...
			//tags = append(tags, field.tags.Name)
			tags[field.tags.Name] = field.typ.Type.String()
		}
	}
	return tags, nil
}
...
// set payload value into field value.
// Return type convert error.
func (f *field) set(payload string) error {
	switch f.val.Kind() {
	case reflect.String:
...
// setValue writes value to fields of a data structure.
// Return error - MultiError.
func (d *data) setValue(mErr *MultiError) error {
	if mErr == nil {
		return errors.New("request: setValue: mErr cannot be nil")
	}
	if d.request == nil {
		return errors.New("request: setValue: httpData cannot be nil")
	}
...
func (f *field) setFileValue(mErr *MultiError) error {
	file := f.request.Files[f.tags.Name]
...
func (f *field) setFilesValue(mErr *MultiError) error {
	files := f.request.Files[f.tags.Name]
...
// setValue writes value to field.
func (f *field) setValue(mErr *MultiError) error {
	if f.typ.Type.Kind() == reflect.Slice {
		f.setArrayValue(mErr)
		return nil
	}
	...
	value := f.request.Values.Get(f.tags.Name)

	if value == "" && f.tags.Tag == requiredTag {
	...
	if f.tags.Tag == optionalTag && value == "" {
		return nil
	}
...
func (f *field) setArrayValue(mErr *MultiError) {
	if f.typ.Type.Elem().Kind() == reflect.Struct {
		val := reflect.New(f.typ.Type.Elem())
		if val.Kind() == reflect.Pointer {
			val = val.Elem()
		}

		typ := val.Type()
		for k, v := range f.request.Arrays {
			_ = k
			_ = v
		}
	...
	requestArray := f.request.Values[f.tags.Name]
	fieldSliceTyp := f.typ.Type
	newSlice := reflect.MakeSlice(fieldSliceTyp, 0, len(requestArray))
	for _, v := range requestArray {
		err := f.addSliceValue(&newSlice, fieldSliceTyp.Elem(), v)
		if err != nil {
			mErr.Add(err)
			return
		}
...
func (f *field) addSliceValue(val *reflect.Value, typ reflect.Type, value string) error {
	switch typ.Kind() {
	case reflect.String:
		*val = reflect.Append(*val, reflect.ValueOf(value))
	case reflect.Int:
		digit, err := strconv.Atoi(value)
		if err != nil {
			return fmt.Errorf("the field '%s' must be a digit; ", f.tags.Name)
		}
		*val = reflect.Append(*val, reflect.ValueOf(digit))
	case reflect.Float64:
		float, err := strconv.ParseFloat(value, 64)
		if err != nil {
			return fmt.Errorf("the field '%s' must be a float digit; ", f.tags.Name)
		}
		*val = reflect.Append(*val, reflect.ValueOf(float))
	default:
		return fmt.Errorf("unknown field type: %s", f.tags.Name)
	}
	return nil
