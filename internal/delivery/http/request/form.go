...
func FormParse(r *http.Request, payload any) error {
	...
	d.httpData, err = parser.HTTPBodyParse(r)
	if err != nil {
		return err
	}
...
func (data *Data) setDataValue(mErr *MultiError) error {
...
func (field *Field) setFileValue(mErr *MultiError) error {
	file := field.httpData.Files[field.tags.Name]
...
func (field *Field) setFilesValue(mErr *MultiError) error {
	files := field.httpData.Files[field.tags.Name]
...
func (field *Field) setDefaultValue(mErr *MultiError) error {
	if field == nil {
	...
	ok := field.httpData.Values.Has(field.tags.Name)
	if !ok && field.tags.Tag == RequiredTag {
		mErr.Add(ErrFieldRequired{field: field.tags.Name})
		return nil
	}

	if field.tags.Tag == OptionalTag && field.httpData.Values.Get(field.tags.Name) == "" {
		return nil
	}

	err := field.setField(field.httpData.Values.Get(field.tags.Name))
	if err != nil {
